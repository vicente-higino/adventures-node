...
export const duelCancelCommand = createBotCommand(
    "cancelduel",
    async (params, ctx) => {
        const { broadcasterId, userDisplayName, broadcasterName, userId, say } = ctx;
        ...
        const result = await handleDuelCancel({ channelProviderId: broadcasterId, channelLogin: broadcasterName, currentUserId: userId, userDisplayName, challengedId });
        say(result);
    },
