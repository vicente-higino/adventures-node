...
// Mutex map per channel for RPS commands
const rpsMutexMap: Map<string, Mutex> = new Map();
function getRpsMutex(channelId: string): Mutex {
    if (!rpsMutexMap.has(channelId)) {
        rpsMutexMap.set(channelId, new Mutex());
    }
    return rpsMutexMap.get(channelId)!;
}
...
export const rpsCommand = createBotCommand(
    ...
    async (params, ctx) => {
        ...
        const mutex = getRpsMutex(broadcasterId);
