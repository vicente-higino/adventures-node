...
export async function grantRedeemable({
    userId,
    channelProviderId,
    redeemableCode,
    quantity = 1
}: GrantRedeemableOptions) {
    const redeemable = await prisma.redeemable.findUnique({
        where: {
            code: redeemableCode
        }
    });
    ...
    return prisma.userRedeemable.upsert({
        where: {
            channelProviderId_userId_redeemableId: {
                channelProviderId,
                userId,
...
type ConsumeRedeemableOptions = {
    userId: string;
    channelProviderId: string;
    redeemableCode: RedeemableCode;
};

export async function consumeRedeemable({
    userId,
    channelProviderId,
    redeemableCode
}: ConsumeRedeemableOptions) {
    const redeemable = await prisma.redeemable.findUnique({
        where: {
            code: redeemableCode
        }
    ...
    await prisma.userRedeemable.update({
        where: {
            id: inventory.id
        },
        data: {
